...
function SQLEditorPage() {
  ...
  const handleConnectToDb = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5002/connect-db", {
        method: "POST",
  ...
  const handleConvertToSql = async () => {
    ...
    try {
      ...
      if (useDbSchema && isConnected) {
        try {
          const schemaResponse = await fetch("http://localhost:5002/get-full-schema");
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5002/execute-sql", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
